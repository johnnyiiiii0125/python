...
def InitialiseRandomBoard(Board, BoardDimension):
    ...
    for Count in range(NumberOfTreasureItems//2):
        ...
        Board = PlaceItem(Board, BoardDimension, DIAMOND)
    return Board
    ...
    for Row in range(BoardDimension):
        for Column in range(BoardDimension):
            if Board[Row][Column] == GOLDCOIN.upper():
                Score = Score + 1
            elif Board[Row][Column] == DIAMOND.upper():
                Score = Score + 2

                DiamondFoundCount = DiamondFoundCount + 1
            if Board[Row][Column] == DIAMOND:
                DiamondNotFoundCount = DiamondNotFoundCount + 1
    if DiamondNotFoundCount == 0:
        Score = Score + 5
    print("The number of diamonds you have found is ", DiamondFoundCount)
    return Score
    ...
    for R in range(StartRow, EndRow + 1):
        for C in range(StartColumn, EndColumn + 1):
            if R != Row or C != Column:
                CharacterRange = CheckCharacter(Board[R][C])
